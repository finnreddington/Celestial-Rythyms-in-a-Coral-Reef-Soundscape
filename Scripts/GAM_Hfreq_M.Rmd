...
left_col <- ggarrange(
  p_time_LFM, p_lunar_LFM, p_current_LFM,
  ncol = 1, align = "v"
)

right_col <- ggarrange(
  p_time_HFM, p_lunar_HFM, p_temp_HFM,
  ncol = 1, align = "v"
)
...
header_plot <- function(txt) {
  ggplot() +
    ...
    theme(plot.margin = margin(0, 0, 4, 0))   # small gap below headers
}

h_left  <- header_plot("Low-freq. amplitude index (M)")
h_right <- header_plot("High-freq. amplitude index (M)")

top_row <- ggarrange(h_left, h_right, ncol = 2, widths = c(1,1))

body <- ggarrange(left_col, right_col, ncol = 2, widths = c(1,1), align = "hv")
...
ggsave("../Plots/Fig3.png", combo_plot, width = 20, height = 12, units = "cm")
